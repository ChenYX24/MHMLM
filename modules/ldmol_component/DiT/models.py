...
class DiTWithTextProj(nn.Module):
    """
    Wrap a DiT model with an (optional) learnable text feature projection.

    This is used to reduce the condition embedding dimension (e.g., Qwen3_8B 4096 -> 1024)
    before feeding it into DiT, to save compute/memory in cross-attention.
    """

    def __init__(self, dit, text_proj=None):
        super().__init__()
        self.dit = dit
    ...
    def forward(self, x, t, y=None, pad_mask=None, **kwargs):
        y = self.text_proj(y)
        return self.dit(x, t, y=y, pad_mask=pad_mask, **kwargs)
    
    def forward_with_cfg(self, x, t, y=None, pad_mask=None, **kwargs):
        y = self.text_proj(y)
        return self.dit.forward_with_cfg(x, t, y=y, pad_mask=pad_mask, **kwargs)
