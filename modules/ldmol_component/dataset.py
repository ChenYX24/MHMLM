...
def split_into_sentences(text: str):
    """
    Lightweight sentence splitter to replace missing utils.split_into_sentences.
    Splits on '.', '!' or '?' followed by whitespace; trims whitespace.
    """
    if not text:
        return []
...
class smi_txt_dataset(Dataset):
    def __init__(self, data_path, data_length=None, shuffle=False, unconditional=False, raw_description=False):
        ...
        self.data = [smi for smi in self.data if Chem.MolFromSmiles(smi) is not None]
